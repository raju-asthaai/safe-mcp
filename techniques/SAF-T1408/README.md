...
date: 2025-11-21
references:
  - https://github.com/saf-mcp/techniques/SAF-T1408
logsource:
  product: mcp
  service: auth
detection:
...
### Preventive Controls
1. **SAF-M-101: Disable Implicit Flow**: Configure authorization server to reject `response_type=token`.  
2. **SAF-M-102: Enforce PKCE**: Require PKCE for all public clients.  
3. **SAF-M-103: Client Registration Hardening**: Allow only authorization code flow with PKCE.  
4. **SAF-M-104: HTTP Message Signatures**: Use RFC 9421 to sign requests, preventing parameter tampering and downgrade attempts.  
5. **SAF-M-105: Audience and Scope Restriction**: Bind tokens to specific resources and minimize privileges.  
6. **SAF-M-106: Secure Token Storage**: Ensure tokens are never stored in URLs, logs, or insecure browser storage.

### Detective Controls
1. **SAF-M-201: Log Monitoring**: Detect requests with implicit flow parameters.  
2. **SAF-M-202: Token Exposure Alerts**: Alert when tokens appear in URL fragments.  
3. **SAF-M-203: Replay Detection**: Monitor for repeated use of the same token across multiple MCP servers.
...
## Related Techniques
- [SAF-T1007](../SAF-T1007/README.md): OAuth Authorization Phishing – Exploit OAuth flows to steal access tokens
- [SAF-T1009](../SAF-T1009/README.md): Authorization Server Mix-up – Client follows redirect to look-alike server
- [SAF-T1202](../SAF-T1202/README.md): OAuth Token Persistence - Theft and reuse of OAuth access/refresh tokens for persistent access
- [SAF-T1308](../SAF-T1308/README.md): Token Scope Substitution - Swaps a limited-scope token with one that has broader permissions
- [SAF-T1507](../SAF-T1507/README.md): Authorization Code Interception - Man-in-the-browser attack steals OAuth authorization codes during the redirect flow
- [SAF-T1706](../SAF-T1706/README.md): OAuth Token Pivot Replay - Reuses OAuth tokens across different services that fail to validate audience claims
