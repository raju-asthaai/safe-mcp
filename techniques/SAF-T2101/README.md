...
A **Data Destruction** attack uses Model Context Protocol (MCP) tools as the execution plane for **wiper-like behaviors**: deleting, overwriting, or otherwise destroying data in file systems, databases, cloud storage, logs, and AI memory so that it becomes unavailable or irrecoverable [1][3][4][5]. Classic destructive malware and wipers erase local or remote data to disrupt operations and sometimes cover tracks [3][7][8]; in SAF-T2101, the same impact is achieved when LLM agents drive over-privileged MCP tools to perform destructive actions.
...
- **MITRE ATTACK**
  - **T1485 – Data Destruction** — adversaries destroy data on local/remote systems to interrupt availability; SAF-T2101 maps MCP-tool-driven destruction to this technique [1][5][10].
...
A typical MCP deployment connects an LLM host to one or more MCP servers that expose tools for interacting with real systems (files, databases, cloud APIs, observability, AI memory, etc.). SAF-T2101 describes how those tools can be turned into **destructive capabilities**:
...
**SAF‑T2101.001 — File-System Level Wipe via MCP Tools.**  
...
**SAF‑T2101.002 — Database/Table Destruction via Query Tools.**  
...
**SAF‑T2101.003 — Cloud Storage / Bucket Purge.**  
...
**SAF‑T2101.004 — AI Memory / Vector Store Destruction.**  
...
### Data Backup & Recovery — Mitigation: SAF‑M‑3: Hardened Data Backup
...
### Logging & Telemetry — Mitigation: SAF‑M‑12: Centralized Logging
...
### Tool Design & Least Privilege — Mitigation: SAF‑M‑10: Principle of Least Privilege
...
### Guardrails & Policy Controls — Mitigation: SAF‑M‑21: Policy Enforcement & Output Isolation
...
### Secure MCP Server Implementation — Mitigation: SAF‑M‑16: Environment Hardening
...
- **Red Team & Chaos Exercises.**  
  Integrate SAF-T2101 scenarios into red team engagements and chaos drills, focusing on loss of availability and recovery posture (how fast can you detect, stop, and restore?) [3][5][7][12][16][32].
...
- **SAF‑T1101 — Command Injection.** Command injection vulnerabilities in MCP servers or tools can provide the foothold used to execute destructive operations.  
- **SAF‑T1104 — Over‑Privileged Tool Abuse.** Overly broad or misconfigured MCP tools make it far easier to enact SAF‑T2101 behaviors once a prompt/agent is compromised.  
- **SAF‑T2102 — Service Disruption.** Data destruction is one of several mechanisms for service-level impact.  
- **SAF‑T2106 — Context Memory Poisoning.** Corrupts data rather than deleting it; often combined with SAF‑T2101 to degrade detection and recovery.
...
|---------|------|---------|--------|
| 1.0 | 2025-11-15 | SAF-T2101 data destruction via MCP tools, sub-techniques, detections, mitigations, and references | Pratikshya Regmi |
