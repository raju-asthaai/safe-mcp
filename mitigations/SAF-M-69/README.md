# SAF-M-69: Out-of-Band Authorization for Privileged Tool Invocations

## Overview
**Mitigation ID**: SAF-M-69  
...
The control addresses the failure mode in which a prompt-manipulated agent decides to invoke a privileged tool and the host executes the call without any independent check. By moving the authorization boundary off the model-reachable surface, SAF-M-69 makes the privileged-invocation decision resistant to natural-language coercion: even a fully compromised model context cannot produce the approval artifact, because the approval is issued by a principal or a service the model cannot impersonate. This mitigation is the load-bearing tool-invocation-time control for SAF-T1309 (Privileged Tool Invocation via Prompt Manipulation); it composes with, but is distinct from, static allowlisting or privilege-tier definitions.

## Mitigates
- [SAF-T1309](../../techniques/SAF-T1309/README.md): Privileged Tool Invocation via Prompt Manipulation
- [SAF-T1104](../../techniques/SAF-T1104/README.md): Over-Privileged Tool Abuse
...
1. **Design Phase**:
   - Inventory tools and mark each as privileged or non-privileged (coordinate with SAF-M-29).
...
3. **Deployment Phase**:
   ...
   - Enable enforcement, monitor denial rates, and iterate. Treat a spike in denials as a potential ongoing SAF-T1309 attempt.

## Benefits
- **Breaks the SAF-T1309 coercion chain**: Since the grant is produced outside the model's context, natural-language manipulation of the model cannot forge it; the worst a compromised context can do is request approval it should not have.
- **Composes with static controls**: Works alongside privilege-tier definitions (SAF-M-29), server allowlisting (SAF-M-14), and token-scope limiting (SAF-M-16) to produce defense in depth across configuration time and invocation time.
...
## Limitations
- **Approval fatigue is a real failure mode**: If users are prompted to confirm too often, they click "approve" without reading, which collapses the control's effectiveness. This is explicitly noted in the OWASP Top 10 for LLM Applications discussion of excessive agency. Mitigate by narrowing the privileged set (SAF-M-14, SAF-M-29), bundling related calls into a single approval, batching repetitive low-risk calls under a short-lived capability token, and designing approval UX that surfaces the distinguishing fields (path, host, SQL, dollar amount) rather than a generic "Allow this tool?" prompt.
...
- **Relies on correct privilege classification**: A tool incorrectly marked non-privileged bypasses this control entirely. Misclassification pushes the failure mode upstream to SAF-M-29.
...
Design rules that make this resistant to SAF-T1309:
...
1. **Security Testing**:
   - Attempt each SAF-T1309 coercion pattern (authority spoofing, debug/simulation pretexting, pre-authorization assertion, role/mode override) against the host and verify that *no* privileged tool executes without a valid out-of-band artifact.
...
- Alert on approval requests for tools this session has never invoked before (possible ongoing SAF-T1309 attempt).
- Alert on capability tokens that verify but reference arguments lexically identical to the preceding user turn (possible dictated-call signal; combine with SAF-T1309 detection guidance and SAF-M-70 baselines).
...
## Related Mitigations
- [SAF-M-29](../SAF-M-29/README.md): Explicit Privilege Boundaries — complementary. SAF-M-29 defines privilege levels, the boundaries between them, and the escalation rules that govern crossing those boundaries. SAF-M-69 supplies the concrete per-invocation authorization artifact (capability token, human-approval signal, policy-engine grant) that M-29's escalation step consumes. Deployments use both: M-29 for the hierarchy and escalation policy, M-69 for the call-time artifact that proves the escalation was granted.
- [SAF-M-14](../SAF-M-14/README.md): Server Allowlisting — narrows the surface of tools that can ever reach the authorization gate, reducing approval-fatigue load on SAF-M-69.
- [SAF-M-16](../SAF-M-16/README.md): Token Scope Limiting — bounds the blast radius of a successfully authorized privileged invocation by ensuring the underlying credentials are minimally scoped.
- [SAF-M-1](../SAF-M-1/README.md): Architectural Defense — Control/Data Flow Separation — complementary upstream control that prevents tool outputs and retrieved content from acting as instructions, reducing the rate at which the authorization gate is asked to adjudicate injection-driven calls.
- [SAF-M-12](../SAF-M-12/README.md): Audit Logging — consumes the grant/deny events emitted by SAF-M-69 for detection and post-incident reconstruction.
