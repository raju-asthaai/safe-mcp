# SAF-M-71: Query Guardrails & Result Limits

## Overview
**Mitigation ID**: SAF-M-71  
...
5. **Compose with post-execution controls**: Guardrails are the first line; SAF-M-23 (truncation), SAF-M-16 (scope-limited credentials), and DLP on outputs remain in place.
...
2. **Development Phase**:
   ...
   - On approved queries, inject a hard `LIMIT` before dispatch when none is present in the submitted SQL; attach a statement-level timeout. Post-execution byte caps on the returned payload are out of scope here — they belong to SAF-M-23.
...
- **Produces clean audit evidence**: Every denied or escalated query is a high-signal event for detection (SAF-T1803 IoC feed).
- **Composable**: Layers cleanly with SAF-M-16 (DB-side least privilege), SAF-M-23 (post-execution truncation), and DLP controls on outputs.
...
- **Per-row sensitivity still needs DLP**: A shape-compliant query can still return sensitive columns. Column-level masking, tokenization, or output DLP — see [SAF-M-72](../SAF-M-72/README.md) — is complementary and required for regulated data.
...
## Related Mitigations
- [SAF-M-23](../SAF-M-23/README.md): Tool Output Truncation — complementary; applies hard caps **after** execution. SAF-M-71 rejects dangerous queries before they run; together they provide defense in depth.
- [SAF-M-16](../SAF-M-16/README.md): Token Scope Limiting — complementary; reduces the DB-side permission surface so that even a guardrail bypass returns less data.
- [SAF-M-29](../SAF-M-29/README.md): Explicit Privilege Boundaries — defines the privilege structure that guardrails enforce at the query layer.
- [SAF-M-72](../SAF-M-72/README.md): Data Loss Prevention on Tool Outputs — complementary; SAF-M-71 blocks overbroad queries at ingress, SAF-M-72 masks or redacts sensitive columns at egress when a query slips through.
