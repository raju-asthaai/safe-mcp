# SAF-M-73: Sampling Budget and Iteration Caps

## Overview
**Mitigation ID**: SAF-M-73  
...
## Mitigates
- [SAF-T1112](../../techniques/SAF-T1112/README.md): Sampling Request Abuse — caps on `maxTokens`, sampling request count, and tool-loop iterations directly bound the quota-bleed, conversation-carryover, and tool-enabled-pivot variants
...
- **Logger**: structured emitter for cap breaches, consumed by [SAF-M-12](../SAF-M-12/README.md) Audit Logging and [SAF-M-11](../SAF-M-11/README.md) Behavioral Monitoring.
- **User notifier**: surfaces cap breaches with provenance via [SAF-M-15](../SAF-M-15/README.md) User Warning Systems.
...
3. **Deployment Phase**:
   ...
   - Monitor cap-breach rates per server as an input to [SAF-M-11](../SAF-M-11/README.md) (Behavioral Monitoring).
...
## Limitations
- **Does not detect subtle bias**: Within-budget malicious sampling (e.g., a single well-crafted prompt that biases later reasoning) is unaffected; pair with [SAF-M-21](../SAF-M-21/README.md) (Output Context Isolation) and [SAF-M-11](../SAF-M-11/README.md) (Behavioral Monitoring).
...
3. **Integration Testing**:
   ...
   - Verify that cap-breach log events are consumed correctly by downstream Behavioral Monitoring and Audit Logging pipelines (SAF-M-11, SAF-M-12).
   - Confirm user-notification handoff to the configured User Warning Systems path (SAF-M-15) on a cap breach.
4. **Observability**:
...
- **Storage**: No persistent storage required for counters (in-memory per session). Cap-breach logs are append-only and size-bounded by log retention policy already in place for SAF-M-12.
...
## Related Mitigations
- [SAF-M-11](../SAF-M-11/README.md): Behavioral Monitoring — consumes cap-breach logs to identify abusive servers
- [SAF-M-12](../SAF-M-12/README.md): Audit Logging — records each sampling request and the budget counters at decision time
- [SAF-M-15](../SAF-M-15/README.md): User Warning Systems — surfaces cap breaches to the user with provenance
- [SAF-M-21](../SAF-M-21/README.md): Output Context Isolation — complements caps by bounding the influence of any output that did get through
- [SAF-M-29](../SAF-M-29/README.md): Explicit Privilege Boundaries — per-server profiles for cap overrides align with per-server privilege tiers
