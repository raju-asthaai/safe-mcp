# SAF-M-72: Data Security - Data Loss Prevention on Tool Outputs

## Overview
**Mitigation ID**: SAF-M-72  
...
Unlike prompt-injection defenses that look for instruction-shaped content in model inputs, SAF-M-72 operates on the **outbound** path of the tool boundary and is specifically tuned to prevent bulk disclosure of sensitive records even when the underlying query is syntactically valid and the agent is behaving as intended. The control composes schema-authoritative filtering (using a data classification catalog that maps columns → labels) with content-based scanners (regex, dedicated validators per identifier class — Luhn / MOD-10 for PAN, MOD-97 for IBAN, format/area checks for US SSN — Named Entity Recognition, ML classifiers, and embedding-based secret detectors) to cover both typed and untyped fields. Every masking or block decision is logged as an audit event, giving defenders a high-signal telemetry source for detecting exfiltration attempts that evade upstream guardrails [1][2][3].

## Mitigates
- [SAF-T1803](../../techniques/SAF-T1803/README.md): Database Dump — primary mitigation; even if query-layer guardrails are bypassed, PII/PCI/secret columns are masked or tokenized before the agent sees the rows, capping the blast radius of a bulk `SELECT`.
- [SAF-T1008](../../techniques/SAF-T1008/README.md): Credential-in-File — secret scanners in the output path redact tokens, keys, and passwords that would otherwise be returned verbatim by file-read or config-dump tools.
- [SAF-T1102](../../techniques/SAF-T1102/README.md): Prompt Injection (Multiple Vectors) — secondary mitigation; bounds the information an injection-hijacked agent can actually exfiltrate through tool results, because sensitive payloads never reach the model context in clear form.
...
```
   ...
   request        └──────────────────────────────────────────────┘
                 ...
                 │  3. Secret Detector (composes SAF-M-63)     │
                 │  4. Policy Engine                            │
...
2. **Development Phase**:
   ...
   - For API-key and credential-like content in outputs, plug in an output-side secret scanner. Many deployments adapt the embedding-based semantic similarity approach from SAF-M-63 (originally scoped to **incoming** credential-seeking queries) by reusing the same similarity engine to score outbound text fragments against known credential patterns. This is a composition pattern, not a direct reuse — the output-side application re-trains or re-threshold-tunes against masked-output fixtures, and SAF-M-63's ingress scope is unchanged.
...
- **Composable with query-layer controls**: stacks cleanly with query shape guardrails (SAF-M-71), row caps (SAF-M-23), and least-privilege DB roles (SAF-M-16) as a defense-in-depth layer.
- **High-signal telemetry**: DLP audit events are dense and rarely benign at scale, making them one of the most reliable detections for SAF-T1803-style exfiltration.
...
| Embedding-based secret scanner (output-side adaptation of the SAF-M-63 approach) | API-key fixtures | 0.94 | 0.96 | Adapts SAF-M-63's ingress similarity technique to output-side recognition [SAF-M-63] |
...
1. **Security Testing**:
   ...
   - Red-team drills simulating SAF-T1803: issue `SELECT *` against a seeded `users` table and confirm no clear-text PII/secret reaches the agent.
...
- **Memory**: ~500 MB–2 GB per worker when loading Presidio/spaCy models; embedding-based secret scanners (SAF-M-63) add ~500 MB.
...
- `dlp.action_count{action=block}` per tool, per caller (primary SAF-T1803 detection).
...
## Related Mitigations
- [SAF-M-71](../SAF-M-71/README.md): Query Guardrails & Result Limits — complementary; SAF-M-71 blocks malformed or overbroad queries at ingress, SAF-M-72 sanitizes results at egress when a query slips through.
- [SAF-M-23](../SAF-M-23/README.md): Tool Output Size Limits — complementary; row/byte caps reduce the volume DLP must scan and bound the worst-case disclosure.
- [SAF-M-63](../SAF-M-63/README.md): Embedding-Based API Key Detection and Filtering — specialization; SAF-M-63 is one high-recall secret-detection strategy that plugs into SAF-M-72's scanner stage.
- [SAF-M-16](../SAF-M-16/README.md): Least-Privilege Tool Authorization — complementary; tokens scope what data is reachable, DLP scopes what of that data is returned in clear.
- [SAF-M-5](../SAF-M-5/README.md): Content Sanitization — distinct control (prompt-injection defense on inbound text); SAF-M-72 is outbound sensitive-data filtering, not instruction filtering.
- [SAF-M-22](../SAF-M-22/README.md): Semantic Output Validation — distinct control; SAF-M-22 checks outputs for instruction-shaped content that could subvert the model; SAF-M-72 checks outputs for sensitive content that should not cross the tool boundary regardless of model behavior.
