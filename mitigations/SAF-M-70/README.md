# SAF-M-70: Detective Control - Tool-Invocation Anomaly Detection & Baselining

## Overview
**Mitigation ID**: SAF-M-70  
...
**Distinction from SAF-M-11 (Behavioral Monitoring).** SAF-M-11 covers LLM behavioral analytics across the entire agent session, including model-generated text, reasoning patterns, tool-usage *sequences*, and conversation-level deviations. SAF-M-70 is narrower: it operates on the single-event tool-invocation log stream, with per-entity statistical baselines over the numeric and categorical features of individual calls (volume, cardinality, destination, time). Deployments typically run both — M-11 for cross-turn behavioral signals, M-70 for per-call tool-stream baselines — with M-70 providing the high-precision per-invocation alerts that feed M-11's broader session-level correlation.

It complements — not replaces — preventive controls such as explicit privilege boundaries (SAF-M-29) and binding authorization; its role is to detect coercion and abuse that slip past prevention, especially for techniques such as SAF-T1309 (privileged tool invocation via prompt manipulation) and SAF-T1803 (database dump via MCP tools) where individual calls can look structurally legitimate.

## Mitigates
- [SAF-T1309](../../techniques/SAF-T1309/README.md): Privileged Tool Invocation via Prompt Manipulation
- [SAF-T1803](../../techniques/SAF-T1803/README.md): Database Dump
...
```text
 ...
 │ (SAF-M-12 substrate) │      │                      │      │  rolling windows)   │
 └───────────────────────┘      └──────────┬───────────┘      └──────────┬──────────┘
                                           │                              │
                                           ▼                              ▼
                                  ┌──────────────────────────────────────────────┐
...
### Prerequisites
- Structured MCP tool-invocation logging in place — see [SAF-M-12: Audit Logging](../SAF-M-12/README.md) — emitting at minimum: timestamp, tenant, user id, agent id, session id, tool name, a serialized form of the arguments (or a structured argument tree) sufficient to compute byte-size and field-cardinality features, result row-count and byte-size, destination (if applicable), and outcome.
...
- **Complements preventive controls**: Fills the gap when prevention (SAF-M-29, authorization gates) is bypassed or misconfigured.
- **Leverages existing log substrate**: Builds on the MCP tool-invocation logs required by SAF-M-12; marginal cost is modeling and alerting, not new instrumentation.
...
# Input: a single tool-invocation event from the SAF-M-12 audit log.
...
def score_event(features, baselines):
    ...
    # Rule A: result_rows far above this entity's historical median -> possible SAF-T1803 bulk export.
    ...
    # Rule B: first-ever use of a privileged tool by this principal -> possible SAF-T1309
...
1. **Security Testing**:
   - Replay synthetic SAF-T1309 traces (privileged tool invoked after a coercion prompt) and verify `privileged_tool.first_use` or argument-cardinality rules fire.
   - Replay synthetic SAF-T1803 traces (wide-table `SELECT` returning 10^6+ rows) and verify result-size robust-z rules fire.
...
## Related Mitigations
- [SAF-M-11](../SAF-M-11/README.md): Behavioral Monitoring — broader LLM behavioral analytics over model outputs and sequences; SAF-M-70 is a specialization focused on the tool-invocation event stream.
- [SAF-M-12](../SAF-M-12/README.md): Audit Logging — provides the structured log substrate on which SAF-M-70 operates; a hard prerequisite.
- [SAF-M-20](../SAF-M-20/README.md): Anomaly Detection (OAuth request patterns) — complementary control at the OAuth/auth layer; SAF-M-70 covers the in-session tool-call layer.
- [SAF-M-29](../SAF-M-29/README.md): Explicit Privilege Boundaries — preventive counterpart for SAF-T1309; SAF-M-70 detects what slips past it.
- [SAF-M-36](../SAF-M-36/README.md): Model Behavior Monitoring — complementary, focused on model inputs/outputs rather than the tool-invocation event stream.
